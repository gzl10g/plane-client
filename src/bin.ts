...
projectMembersCmd
  ...
  .option("-p, --project <ref>", "Project UUID or identifier (e.g. PCL)")
  .option("--yes", "Skip the confirmation prompt")
  .action(async (membershipId: string, opts: { project?: string; yes?: boolean }) => {
    await runHandler(async () => {
      const { handleProjectMembersDeactivate } = await import("./cli/members.js");
      await handleProjectMembersDeactivate(membershipId, { project: opts.project, yes: opts.yes });
    });
  });
...
invitationsCmd
  ...
  .option("--yes", "Skip the confirmation prompt")
  .action(async (invitationId: string, opts: { yes?: boolean }) => {
    await runHandler(async () => {
      const { handleInvitationsDelete } = await import("./cli/members.js");
      await handleInvitationsDelete(invitationId, { yes: opts.yes });
    });
  });
