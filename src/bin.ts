...
program
  ...
  .action(async (projectRef: string) => {
    await runHandler(async () => {
      ...
      await handleUseProject(projectRef);
    });
  });
...
projectsCmd
  ...
  .action(async (ref: string, opts: { confirm?: string; dryRun?: boolean }) => {
    await runHandler(async () => {
      const { handleProjectsDelete } = await import("./cli/projects.js");
      await handleProjectsDelete(ref, { confirm: opts.confirm, dryRun: opts.dryRun });
    });
  });
...
attachmentsCmd
  ...
  .option("-p, --project <ref>", "Project UUID or identifier (e.g. PCL)")
  .action(async (workItemId: string, attachmentId: string, opts: { project?: string }) => {
    await runHandler(async () => {
      const { handleAttachmentsDelete } = await import("./cli/attachments.js");
      await handleAttachmentsDelete(workItemId, attachmentId, { project: opts.project });
    });
  });
