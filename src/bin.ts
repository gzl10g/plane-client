...
workItemsCmd
  ...
  .option("--json", "Output as JSON")
  .action(async (opts: { project?: string; perPage?: string; orderBy?: string; expand?: string; withModules?: boolean; json?: boolean }) => {
    await runHandler(async () => {
...
workItemsCmd
  ...
  .option("--json", "Output as JSON")
  .action(async (identifier: string, opts: { withModules?: boolean; json?: boolean }) => {
    await runHandler(async () => {
      const { handleWorkItemsGet } = await import("./cli/work-items.js");
      await handleWorkItemsGet(identifier, { withModules: opts.withModules, json: opts.json ?? program.opts().json });
    });
  });
...
workItemsCmd
  ...
  .option("--with-modules", "Attach module membership client-side (Plane API ignores expand=modules; walks every module in the project)")
  .option("--json", "Output as JSON")
  .action(async (uuid: string, opts: { project?: string; withModules?: boolean; json?: boolean }) => {
    await runHandler(async () => {
      const { handleWorkItemsGetById } = await import("./cli/work-items.js");
      await handleWorkItemsGetById(uuid, { project: opts.project, withModules: opts.withModules, json: opts.json ?? program.opts().json });
    });
  });
...
workItemsCmd
  ...
  .option("--json", "Output as JSON")
  .action(async (opts: { project?: string; name: string; priority?: string; state?: string; descriptionHtml?: string; descriptionHtmlFile?: string; labels?: string; assignees?: string; module?: string; json?: boolean }) => {
    await runHandler(async () => {
...
workItemsCmd
  ...
  .option("--description-html-file <path>", "Read description HTML from a file (for payloads too large for argv)")
  .option("--json", "Output as JSON")
  .action(async (id: string, opts: { project?: string; name?: string; priority?: string; state?: string; descriptionHtml?: string; descriptionHtmlFile?: string; json?: boolean }) => {
    await runHandler(async () => {
