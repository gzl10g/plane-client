...
export async function handleProjectsList(
  ...
  const perPage = parseCount(opts.perPage, "--per-page", 100);
...
async function resolveProjectRef(
  ...
  if (UUID_RE.test(ref)) {
    ...
    if (project === null) throw new NotFoundError(`Project not found: ${ref}`);
    return project;
  }
...
export async function handleProjectsDelete(
  ref: string,
  opts: { confirm?: string; dryRun?: boolean; json?: boolean },
  deps?: HandlerDeps,
): Promise<void> {
  const client = resolveClient(deps);
  const project = await resolveProjectRef(client, ref);

  const inventory = await countContents(client, project.id, project);
  const summary =
  ...
  if (opts.dryRun) {
    reportAction(opts, `Would delete:\n${summary}`, {
      dryRun: true,
  ...
  reportAction(opts, `Deleted project ${project.identifier} (${project.id})`, {
    deleted: { id: project.id, identifier: project.identifier },
  });
}
...
async function countContents(
  ...
  const [workItems, modules, cycles, intake] = await Promise.all([
    ...
    (async () => {
      // A project with the intake toggle off answers this with an empty list
      // rather than an error, so the count degraded to a confident `0` while
      // the queue held real items — verified on a project with 2 intake issues
      // and the toggle disabled. Since the inventory is the only thing holding
      // up the confirmation ceremony, "0" has to mean zero. When the feature is
      // off, the honest answer is that we cannot see it.
      if (project.intake_view === false) return "? (intake disabled — cannot be listed)";
      try {
...
async function promptForIdentifier(identifier: string | undefined): Promise<string> {
  // Same trap as confirmAction: with no terminal this used to hang for ever and
  // die with exit 13. `projects delete` deliberately has no bare `--yes`, so the
  // escape hatch to name here is `--confirm <IDENTIFIER>`.
  if (!process.stdin.isTTY) {
    throw new Error(
      `Refusing to prompt: stdin is not a terminal. Pass --confirm ${identifier ?? "<IDENTIFIER>"} to run non-interactively.`,
    );
  }
