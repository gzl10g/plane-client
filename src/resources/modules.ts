...
export class ModulesResource {
  ...
  /**
   * Lists one page of modules.
   * @param projectId - Project UUID
   * @param options - Pagination options
   * @returns Paginated modules
   */
  async listPage(projectId: string, options?: ListOptions): Promise<Page<Module>> {
    ...
    const data = await this.request<{
      ...
      next_page_results?: boolean;
      total_results?: number;
    }>(`/projects/${projectId}/modules/`, { params, signal: options?.signal });
    if (!data) return { items: [], hasNext: false };
    return toPage<Module>(data);
  }

  /**
   * Iterates every module in the project across all pages.
   * @param projectId - Project UUID
   * @param options - List options (cursor managed automatically)
   * @yields Module one at a time
   */
  async *listAll(
    projectId: string,
    ...
    do {
      ...
      for (const module of page.items) yield module;
      cursor = page.nextCursor;
    } while (cursor);
  }

  /**
   * Lists all modules in a project, walking every page. It used to return the
   * first page only — which also silently narrowed `membershipMap()`, and with
   * it `--with-modules`, to whatever modules fitted on that page.
   * @param projectId - Project UUID
   * @returns Array of every module in the project
   */
  async list(projectId: string): Promise<Module[]> {
    const modules: Module[] = [];
    for await (const module of this.listAll(projectId)) modules.push(module);
    return modules;
  }
   ...
   * @returns The module
   */
  async get(projectId: string, moduleId: string): Promise<Module | null> {
    ...
    try {
      return await this.request<Module>(`/projects/${projectId}/modules/${moduleId}/`);
    } catch (err) {
