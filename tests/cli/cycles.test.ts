...
describe("cycles CLI handlers", () => {
  ...
  // F10: delete a cycle, guarded by confirmation unless --yes.
  describe("handleCyclesDelete", () => {
    it("deletes the cycle and confirms when --yes is passed", async () => {
      ...
      await handleCyclesDelete("c1", { yes: true }, {
        client: spyClient,
        config: mockConfig,
      });

      assert.equal(deletedWith.id, "c1");
      assert.equal(deletedWith.project, mockConfig.project);
      assert.ok(consoleLogOutput.some((l) => l.includes("Deleted cycle c1")));
    });
  });
