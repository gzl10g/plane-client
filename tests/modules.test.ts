...
describe("ModulesResource pagination", () => {
  ...
  it("modules list walks every page instead of stopping at the first", async () => {
    ...
    try {
      const items = await client.modules.list("p1");
  ...
  it("modules list does not follow the cursor the last page still carries", async () => {
    const mock = mockFetch([
