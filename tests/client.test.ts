...
describe("PlaneClient", () => {
  ...
  describe("version", () => {
    it("returns instance version from /api/instances/", async () => {
      const mock = mockFetch([{ status: 200, body: { instance: { current_version: "1.2.3" } } }]);
      try {
        const version = await client.version();
        assert.equal(version, "1.2.3");
        assert(mock.calls[0].url.includes("/api/instances/"));
        assert(!mock.calls[0].url.includes("/api/v1/"));
      } finally { mock.restore(); }
    });

    it("returns 'unknown' when version field is missing", async () => {
      const mock = mockFetch([{ status: 200, body: { instance: {} } }]);
      try {
        const version = await client.version();
    ...
    it("throws PlaneApiError on failure", async () => {
      const mock = mockFetch([{ status: 500, body: "error" }]);
      try {
        await assert.rejects(
          () => client.version(),
          (err: unknown) => {
