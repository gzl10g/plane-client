...
describe("WorkItemsResource", () => {
  ...
  it("get por identifier usa endpoint directo /work-items/{ident}/", async () => {
    ...
    try {
      ...
      assert(url.includes("expand=state%2Cmodules"));
  ...
  it("getById usa /projects/{p}/work-items/{uuid}/", async () => {
    ...
    try {
      ...
      assert(mock.calls[0].url.includes("expand=state%2Cmodules"));
      assert.equal(res?.id, "wi-1");
    } finally {
      mock.restore();
    }
  ...
  it("relations.list returns RelationsMap", async () => {
    const mock = mockFetch([{
      status: 200,
      body: {
        blocking: [{ project_id: "p1", issue_id: "wi-2" }],
        blocked_by: [],
        duplicate: [],
        relates_to: [],
        start_before: [],
    ...
    try {
      ...
      assert.deepEqual(relations.blocking, [{ project_id: "p1", issue_id: "wi-2" }]);
  ...
  it("stops paginating when next_page_results is false, even though a cursor comes back", async () => {
    // The real API answers the LAST page with `next_page_results: false` AND
    // `next_cursor: "100:1:0"` (verified live against 1.4.1), and following that
    // cursor returns `count: 0` with yet another cursor behind it, for ever. The
    // old mock hid this by omitting the cursor on the last page — a mock kinder
    // than the API, which is exactly what this repo warns about.
    const mock = mockFetch([
      { status: 200, body: { results: [{ id: "1" }], next_cursor: "100:1:0", next_page_results: false } },
    ...
    try {
      ...
      const items: unknown[] = [];
      for await (const item of client.workItems.listAll("p1")) items.push(item);
