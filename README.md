...
- **`work-items get` / `get-by-id`** request `expand=state,modules`, so the response carries the expanded `state` object and `module_ids`. `work-items list` defaults to `--expand state,modules`; pass `--expand <fields>` to override.
...
| `states` | list |
| `labels` | list, create |
| `modules` | list, get, create, update, delete, workItems, workItemsAll, addWorkItems, removeWorkItem |
